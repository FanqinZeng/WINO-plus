...
@torch.no_grad()
def decoding_wino_with_trajectory(
    model,
    prompt: torch.Tensor,
    gen_length: int = 256,
    ...
    mask_id: int = 126336,
    threshold: float = 0.5,
    threshold_back: float = 0.8,
) -> tuple[torch.Tensor, int, list[int], list[int]]:
    """WINO decoding for LLaDA with accepted/proposed trajectory tracking."""
    ...
    trajectory_accept = torch.full((1, x_block.shape[1]), -1, dtype=torch.long, device=device)
    proposal_history: list[dict[int, int]] = [{} for _ in range(x_block.shape[1])]
    ...
    for num_block in range(num_blocks):
        ...
        while mask_index_block.any():
            current_global_step = step + block_step
            ...
            buffer_start = x_block.shape[1] - block_length
            predicted_tokens = x0[0, buffer_start : buffer_start + block_length].tolist()
            for offset, token_id in enumerate(predicted_tokens):
                real_pos = current_start + offset
                proposal_history[real_pos].setdefault(int(token_id), current_global_step)
            ...
            trajectory_accept[transfer_index] = current_global_step
            ...
            trajectory_accept[remask_index_shift] = -1
    ...
    output_ids = x_block[:, : prompt_len + gen_length]
    final_ids = output_ids[0].tolist()
    trajectory_accepted = trajectory_accept[0, prompt_len : prompt_len + gen_length].tolist()
    trajectory_proposed = []
    for pos in range(prompt_len, prompt_len + gen_length):
        token_id = final_ids[pos]
        trajectory_proposed.append(proposal_history[pos].get(token_id, -1))

    return output_ids, step, trajectory_accepted, trajectory_proposed
