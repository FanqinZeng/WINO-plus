...
from prepare_trainingdata.common.eval_utils import countdown_is_correct, extract_countdown_answer
    ...
    with Path(output_file).open("w", encoding="utf-8") as f:
        for local_index, row in enumerate(tqdm(rows, position=rank, desc=f"worker {rank}", leave=False)):
            ...
            for temperature in args_dict["temperatures"]:
                ...
                extracted = extract_countdown_answer(generated_text)
                correct = countdown_is_correct(extracted, row.get("digital_answer"), row.get("numbers", []))

                final_record = normalize_trajectory_record(
                    {
                        "unique_id": row.get("unique_id", f"countdown_{start_index + local_index}"),
                        "source": "countdown",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect Countdown WINO trajectories with LLaDA.")
    parser.add_argument("--model-path", required=True)
    parser.add_argument("--input-file", required=True, help="Processed Countdown JSONL from prepare_countdown.py.")
    ...
    print(
        f"Saved {total} Countdown trajectories to {output_path}; "
        f"correct={correct}; accuracy={correct / total if total else 0:.4f}"
    )
